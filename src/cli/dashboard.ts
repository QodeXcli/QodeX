...
export function buildDashboardHtml(d: DashboardData, opts: { token?: string } = {}): string {
  ...
  const maintainPanel = `<div class="panel"><div class="ctl" style="border:0;padding:0 0 12px"><h2 style="margin:0">Maintain status — self-improvement</h2>${live ? `<button onclick="act('maintain.preview',{})">🔍 Run maintain now (preview)</button>` : ''}</div>${ms && ms.totalRuns > 0 ? `
    ...
    ${d.maintainForecast ? `<div class="ctl"><span>Trend forecast</span><span class="dim">${d.maintainForecast.direction === 'rising' ? '<b class="ok">rising ↑</b>' : d.maintainForecast.direction === 'falling' ? '<b class="warn">cooling ↓</b>' : 'steady →'} · avg ~${d.maintainForecast.weeklyAvg}/wk · next week ≈ ${d.maintainForecast.nextWeek}</span></div>` : ''}
  ...
  const extractPanel = em && em.truncated > 0 ? `<div class="panel"><h2>Web extract — semantic vs positional</h2>
    <div class="cards" style="grid-template-columns:repeat(3,1fr);margin:0 0 12px">
      ${card('semantic hits', String(em.semantic), 'var(--green)')}
      ${card('head+tail fallback', String(em.headTail), 'var(--amber)')}
      ${card('semantic hit-rate', `${Math.round(em.semanticRate * 100)}%`, em.semanticRate >= 0.5 ? 'var(--green)' : 'var(--dim)')}
    </div>
    <div class="ctl" style="border:0"><span class="dim">Of ${em.truncated} oversized page(s), ${em.semantic} returned the passages most relevant to the query instead of a blind head+tail window.</span></div>
  </div>` : '';
...
export async function gatherDashboardData(cwd: string): Promise<DashboardData> {
  ...
  const maintain = await (async () => {
    try {
      ...
      const { buildMaintainStats, weeklyReport, recommendNextScope, trendByWeek, projectMonthly, forecastTrend } = await import('./maintain-stats.js');
      const store = getScheduleStore();
      ...
      const now = Date.now();
      return { stats, weekly: weeklyReport(mruns, now), next: recommendNextScope(mruns, stats, MAINTAIN_SCOPES), trend: trendByWeek(mruns, now), projection: projectMonthly(mruns, now), forecast: forecastTrend(mruns, now) };
  ...
  const extractMetrics = await (async () => {
